...
確認できなかった点は「未確認」と書き、断定しない。先方の β 版は見ていないので、以下は「一般に、こう決める必要がある」という提案で、先方の現状の評価ではない。
...
- **確認できなかったこと（未確認）**: **緯度経度そのものを何日まで保存できるか**。利用規約（Service Specific Terms）の本文を、今回のツールでは取得できなかった。**「期間はこれ」とは書かない。案件に入る前に、規約の該当条項を直接読んで確認する必要がある。**
